...
export const portRef = z.union([
  ...
  z.custom<{ getPortSelector: () => string }>(
    ...
    {
      message: "Port reference must be either a string selector (e.g. '.ComponentName > .portName') or a valid Port object"
    }
  ),
], {
  description: "Port Reference",
  invalid_type_error: "Port reference must be either a string selector or Port object with getPortSelector() method"
})
...
export const traceProps = z.union([
  baseTraceProps.extend({
    path: z.array(portRef).min(2, "Path must contain at least 2 port references"),
  }),
  baseTraceProps.extend({
  ...
  invalid_type_error: "Trace must specify either a 'path' array or 'from'/'to' pair of port references"
})
